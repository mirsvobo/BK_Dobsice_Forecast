...
class ForecastModel:
    def __init__(self, best_params=None):
        ...
        if torch.cuda.is_available():
            ...
            gc.collect()
            print(f"   [ForecastModel] GPU: {torch.cuda.get_device_name(0)} | Batch: {self.params['batch_size']}")

    def _build_model(self, callbacks=None):
        """
        Sestaví instanci modelu TFT s maximálním výkonem.
        """
        trainer_args = {
            'max_steps': self.params['max_steps'],
            'accelerator': self.accelerator,
            'precision': '16-mixed',  # 2x méně paměti, rychlejší výpočty na RTX

            # Vypnutí validace pro maximální rychlost průchodu
            'limit_val_batches': 0,
        ...
        tft_model = TFT(
            ...
            # 0 je na Windows nejbezpečnější a nejrychlejší pro start
            num_workers_loader=0,
            drop_last_loader=False,

            random_seed=42,
            optimizer_kwargs={'lr': self.params['learning_rate']},
    ...
    def train(self, df_sales, df_guests, ui_callback_cls=None, ui_container=None):
        ...
        if ui_callback_cls and ui_container:
            ...
            cb = ui_callback_cls(total_steps=self.params['max_steps'], container=st_cont)
